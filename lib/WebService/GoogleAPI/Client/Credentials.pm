...
$WebService::GoogleAPI::Client::Credentials::VERSION = '0.22';
...
has 'user' => (is => 'rw', trigger => \&get_access_token_for_user)
  ;    # full gmail, like peter@shotgundriver.com
has 'auth_storage' => (
  is      => 'rw',
  default => sub { WebService::GoogleAPI::Client::AuthStorage->new }
...
sub get_scopes_as_array {
  ...
  if ($self->auth_storage->is_set) {
    ...
    croak
      q/Can't get access token for specified user because storage isn't set/;
  }
