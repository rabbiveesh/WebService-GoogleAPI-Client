...
$WebService::GoogleAPI::Client::UserAgent::VERSION = '0.22';
  ...
  if (($res->code == 401) && $self->do_autorefresh) {
    if ($res->code == 401) {   ## redundant - was there something else in mind ?
      ...
      croak
        ...
        unless $self->user;
      cluck
"401 response - access_token was expired. Attemptimg to update it automatically ..."
        if ($self->debug > 11);
      ...
      cluck "validated_api_query() Got a new token: " . $new_token
        if ($self->debug > 11);
      ...
      $res = $self->start($self->build_http_transaction($params))
...
sub refresh_access_token {
  ...
  if (
    ...
    )    ## could this be caught somewhere earlier than here?
    || (!defined $credentials->{client_secret}
    )    ##  unless credentials include an access token only ?
    || (!defined $credentials->{refresh_token})
  ) {
    croak
'If you credentials are missing the refresh_token - consider removing the auth at '
...
"Not enough credentials to refresh access_token. Check that you provided client_id, client_secret and refresh_token";
  ...
  cluck "refresh_access_token:: Attempt to refresh access_token "
    if ($self->debug > 11);
  $credentials->{grant_type} = 'refresh_token';
  return $self->post(
    'https://www.googleapis.com/oauth2/v4/token' => form => $credentials)
    ->res->json || croak('refresh_access_token failed');    # tokens
}
