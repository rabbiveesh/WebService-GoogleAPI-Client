...
$WebService::GoogleAPI::Client::VERSION = '0.22';
use Data::Dump qw/pp/;
use Moo;
...
sub api_query {
  ...
  @teapot_errors =
    $self->_process_params_for_api_endpoint_and_return_errors($params)
...
sub _ensure_api_spec_has_defined_fields {
  ...
  foreach my $expected_key (
    qw/path title ownerName version id discoveryVersion revision description documentationLink rest/
  ) {
...
sub _process_params_for_api_endpoint_and_return_errors {
  ...
  $api_discovery_struct->{baseUrl} =~
    s/\/$//sxmg;    ## remove trailing '/' from baseUrl
...
sub has_scope_to_access_api_endpoint {
  ...
  if (keys(%$method_spec) > 0)    ## empty hash indicates failure
  {
    my $configured_scopes =
      $self->ua->get_scopes_as_array();    ## get user scopes arrayref
    ## create a hashindex to facilitate quick lookups
    my %configured_scopes_hash = map { s/\/$//xr, 1 }
      @$configured_scopes
      ; ## NB r switch as per https://www.perlmonks.org/?node_id=613280 to filter out any trailing '/'
    my $granted =
      0;    ## assume permission not granted until we find a matching scope
    ...
    return
      0
      ; ## cannot get method spec - warnings should have already been issued - returning - to indicate access denied
  }
