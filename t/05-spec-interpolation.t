...
my $default_file = $ENV{'GOOGLE_TOKENSFILE'}
  || "$dir/../../gapi.json"
  ;    ## assumes running in a sub of the build dir by dzil
$default_file = "$dir/../gapi.json"
  unless -e $default_file
  ; ## if file doesn't exist try one level up ( allows to run directly from t/ if gapi.json in parent dir )

#if running from root of the repo, grab the one from the t/ directory
$default_file = "$dir/t/gapi.json" unless -e $default_file;

plan skip_all =>
'No service configuration - set $ENV{GOOGLE_TOKENSFILE} or create gapi.json in dzil source root directory'
  unless -e $default_file;

ok(
  my $gapi = WebService::GoogleAPI::Client->new(
    debug     => $DEBUG,
    gapi_json => $default_file
  ),
  'Creating test session instance of WebService::GoogleAPI::Client'
);
...
my $options = {
  ...
  options         => { fields => 'files(id,name,parents)' }
};
...
$options = {
  ...
  options         =>
    { spreadsheetId => 'sner', ranges => ['Sheet1!A1:A2', 'Sheet1!A3:B5'], },
};
build_req($options);
is $options->{path},
...
my @errors;
subtest 'Checking for proper failure with {+params} in unsupported ways' =>
  sub {
  plan skip_all => <<MSG
Need access to the scope https://www.googleapis.com/auth/jobs 
or https://www.googleapis.com/auth/cloud-platform
